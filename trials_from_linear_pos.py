...
# -*- coding: utf-8 -*-
"""
Created on Mon Aug  8 10:29:20 2022
...
def load_data(path):
    ...
    file = [f for f in listdir if  'LinearPos_' + name in f]
    print('Using ', file)
    lin_pos = pd.read_csv(path + '/' + file[0], index_col=['Time (s)'])
    return lin_pos 


def segment_departure(idx_dep_off, idx_dep_on, trials, min_position):    
...
def make_trials(dir_data: str, events: List, 
    ...
    Args:
        dir_data (str): the directory of the folder that contains Analysis 
    ...
    """    
    lin_pos_tsd = load_data(path)
    lin_pos_tsd = nap.TsdFrame(lin_pos_tsd)
    
    # plt.plot(position['x'].restrict(epochs['wake'].loc[[0]]),position['z'].restrict(epochs['wake'].loc[[0]]),'.')
    
    pos_time = position.index

    # We only want positions of the current ep, so we restrict the Tsd
    lin_pos_restricted = lin_pos_tsd.restrict(position.time_support.loc[[0]]) 
    ...
    while idx_dep_off < nbBins:
        ...
        trial_times['fw_dep_start'] = trials.loc[idx_start, 'Time (s)']
        trial_times['fw_dep_end'] = trials.loc[idx_dep_off, 'Time (s)']
        ...
        trial_times['fw_arm_start'] = trials.loc[idx_dep_off, 'Time (s)']
        ...
        if max_position >= right_threshold:
            ...
            trial_type = 1
            # when did he leave the new site?
            ...
            trial_times['fw_arm_end'] = trials.loc[idx_reward-1, 'Time (s)']                 
            trial_times['bw_arm_start'] = trials.loc[idx_reward_off, 'Time (s)']
            trial_times['bw_arm_end'] = trials.loc[idx_dep_on, 'Time (s)']
            ...
            # when did he leave the reward site?
            idx_reward_off = idx_reward
        ...
        if trial_type is None:
            ...
            trial_times['bw_dep_start'] = trials.loc[idx_dep_on, 'Time (s)']
            trial_times['bw_dep_end'] = trials.loc[min_position, 'Time (s)']
...
make_trials(dir_data=path, events=[1,3], right_threshold=2.93, left_threshold=1.93) 